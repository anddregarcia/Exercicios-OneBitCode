...
export function Items() {
  ...
  const handleSave = async () => {
    try {
      if (activeTab === "item") {
        if (!itemName || itemBrandIds.length === 0 || !itemCategory || !itemUnit || !itemPackaging || !itemPackageSize) {
              ...
              {items.map((item) => (
                <div key={item.id} className="flex items-center justify-between rounded-lg border border-border p-4">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{getBrandNames(item.brandIds || [])} • {categories.find((c) => c.id === item.categoryId)?.name || "Sem categoria"}{item.isVegan ? " • Vegano" : ""}</p>
                  </div>
